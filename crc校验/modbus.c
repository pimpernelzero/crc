...
unsigned int CrcValueByteCalc(const unsigned int* data, unsigned int length)  // 已知最佳
{
...
unsigned int CrcValue(const unsigned int* data, unsigned int length)  // 已知最佳
{
	...
	unsigned long uIndex;                      	// CRC分械乃饕?
	while (length--)                  	// 传输消息缓冲区  
	{
...
unsigned int crc_chk(unsigned int* data_value, unsigned int length)
{
	...
	return (crc_value << 8 | crc_value >> 8);
}
